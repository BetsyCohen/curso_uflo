...
subtitle: "Primeros pasos en R para Sociología - Clase 2"
author: "Betsy Cohen"
...
-   **Ventajas:**
    ...
    -   **Documentados:** Podemos agregar comentarios (#) para explicar el código.
...
En el cuadrante de Output (abajo a la derecha) tenemos varias pestañas entre ella una que vamos a usar bastante la pestaña
**help**
Gracias al trabajo de la comunidad vas a notar que en general la documentación y ayuda de las funciones siempre te dan una descripción, detalles y ejemplos de usos.

:::: {.columns}
::: {.column width="50%"}
![](images/clase_2_parte_2_screenshot_help_1)
:::
::: {.column width="50%"}
![](images/clase_2_parte_2_screenshot_help_2)
:::
:::: 
...
Para que podamos guardar y luego "llamar" nuestro vector vamos a necesitar ponerle un nombre. 
Y para ello en R utilizamos el símbolo **<-** o también cariñosamente le decimos **flechita**
...
Entonces si realizamos una **operación entre dos vectores** se ejecutará entre los vectores con misma ubicación.
Por ejemplo:
...
![](images/clase_2_parte_2_meme_para_para_para.jpg)

¿A donde van los vectores que creamos? 

**Probalo** en tu consola y tu IDE de R y después volvé a esta presenta

## 

![](images/clase_2_parte_2_a_few_moments_later.jpg)
...
R nops muestra el **nombre** del vector (en este caso `edades`) y el tipo o **clase** que en este caso es `numeric`.
...
## Vectores Numéricos: Cantidades y Medidas 📊
...
Aprovechandon nuestro vector `edades` Podriamos preguntarle a R la edad de nuestro entrevistado más joven. 
Para eso vamos a usar la función `min()`
...
¿Qué representá cada una? ¿Qué conclusiones sacarías acerca de esta muestra?
Podes usar la ventana de **ayuda** integrada dentro de R ubicada en el panel inferior

:::

### Vectores de Caracteres:  Texto y Etiquetas 💬
...
::: {.callout-important}
## ¡OJO!
Los strings no necesariamente tienen que contener letras. También pueden tener números almacenados como texto.
Por ejemplo:
```{r}
 c("10", "9", "8", "10", "9")
```
...
### Vectores Lógicos:  Verdadero o Falso ✅ ❌
...
### Vectores Factor:  Categorías con Sentido 🗂️
...
Si miras el Global enbviroment ¿qué diferencias encontras entre niveles_educativos_caracter y niveles_educativos_factor?
:::
...
### Creando Data Frames con `data.frame()`

Podemos crear data frames combinando vectores:
...
!()[images/clase_2_parte_2_screenshot_tidy_shape.png]

###  Seleccionando Partes de un Data Frame  ✂️

Podemos acceder a partes específicas de un data frame usando corchetes `[]` o el símbolo `$`(AKA pesitos):
...
**Ejemplos con vectores numéricos:**
...
## 2. Operadores Relacionales:  Comparando Datos  > < == !=
...
## 3. Operadores de Asignación:  Guardando Valores en Objetos  <-  =  ->

- **`<-` (Asignación "flecha"):**  Asigna el valor de la derecha al objeto de la izquierda.  **¡Es el más común y recomendado!**
