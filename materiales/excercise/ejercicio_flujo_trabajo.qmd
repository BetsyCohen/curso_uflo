...
::: {.callout-idea}
**Si bien es una cuestión de gustos**
- Usa str() cuando necesites un desglose estructural rápido y conciso.
- Usa glimpse() cuando quieras una vista más amigable y horizontal, especialmente con data frames grandes.
:::
...
6.  **Explora `encuesta_online` con `head()` y `tail()`:**
    Usa `head()` y `tail()` para ver las primeras y últimas filas del data frame `encuesta_online`. Prueba a cambiar el argumento `n` para ver más o menos filas (ej: `head(encuesta_online, n = 15)`).
...
7.  **Explorá `encuesta_online` con `summary()`:**
